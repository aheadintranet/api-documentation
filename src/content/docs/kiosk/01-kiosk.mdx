...
| fullHtmlContent | If sent with the value **true**, sends all text of a news in its HTML-format                    | No, default is **false** |
...
:::note
A response is cached for **15 minutes**.
:::
...
[
  {
    ...
    "htmlContent": "<h3><span>Greetings y'all</span></h3><p><span>Welcome to our </span><b><strong>seasonal greetings</strong></b><span>.</span></p>"
  }
]
```
...
| leadText                      | The first n characters of the content of the news, depending on the provided **maxLeadText**. This is provided in plain text.   | Yes         |
| htmlContent                   | If `fullHtmlContent` query value was provided, it contains all text of a news in its HTML-format.                              | Yes |
...
:::note
If present, the `htmlContent` does not contain a full HTML document, but rather HTML fragments that can be placed e.g. into the body-tag
of a document.
:::