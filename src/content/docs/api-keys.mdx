...
:::note
An API-Key will only work for its intended usage. You cannot e.g. use one created for "Activities" for
the "People Import".
:::
...
:::note
It is also possible to provide the API-Key as a query parameter named **code**. However, bear in mind that any infrastructure that logs
requests will see the API-Key as part of the URLs metadata.
:::